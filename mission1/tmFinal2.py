...
# ----------------------------------------------------------------------
# 0. GLOBAL CONFIGURATION & SCHEMAS (Unchanged prompts for context)
# ----------------------------------------------------------------------
...
You MUST return ONLY a single JSON object matching this exact schema:
{
  ...
  "candidate_years_extracted": <float: total years of relevant domain experience found in the CV, rounded to two decimal places>,
...
# Dynamic RABCC Auditor Prompt Template with structural format guardrails
...
class BaseAgent:
    def __init__(self, client: OpenAI, system_prompt: str, force_json: bool = False):
        self.client = client
...
class ExecutorAgent:
    def __init__(self, client: OpenAI):
        self.skills_agent = BaseAgent(client, SKILLS_EXECUTOR_PROMPT, force_json=True)
        self.exp_agent = BaseAgent(client, EXPERIENCE_EXECUTOR_PROMPT, force_json=True)
...
# ----------------------------------------------------------------------
# 3. NEW DECOUPLED INFRASTRUCTURE CLASSES (SRP Refactoring)
# ----------------------------------------------------------------------
...
class MultiAgentJobMatcher:
    ...
    def extract_metrics(self, job_text: str, cv_text: str, industry_type: str = "Tech") -> List[Dict]:
        ...
        # 2. Execute Orcheined Sub-loop Processing Pipelines
...
# ----------------------------------------------------------------------
# 5. SCORING ENGINE & RUNTIME PLATFORM (Unchanged)
# ----------------------------------------------------------------------
